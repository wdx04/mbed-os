...
#define IOMUXC_PAD_CONTROL_OFFSET 0x0B0

/* Get the IOMUX register address from the pin number */
static uint32_t get_iomux_reg_base(PinName pin)
{
    int32_t gpio_port = pin >> GPIO_PORT_SHIFT;
    ...
    {
    case 1:
        if ((gpio_pin >= 0) && (gpio_pin < 14)) {
            retval = 0x401F80BC - (gpio_pin * 4);
        } else if ((gpio_pin >= 14) && (gpio_pin < 29)) {
            retval = 0x401F8048 - ((gpio_pin - 14) * 4);
        }
        break;
    case 2:
        if ((gpio_pin >= 0) && (gpio_pin < 14)) {
            retval = 0x401F8084 - (gpio_pin * 4);
        }
        break;
    }
    return retval;
...
void pin_function(PinName pin, int function)
{
    ...
    uint32_t muxregister = get_iomux_reg_base(pin);
