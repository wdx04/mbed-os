...
#ifndef USBPHYHW_RA6M3_H
#define USBPHYHW_RA6M3_H
...
/**
 * \brief USB Phy driver for the Renesas RA6M3 USB_FS0 (USB 2.0 Full-Speed)
 * peripheral controller.
 *
 * The RA6M3 USB_FS0 peripheral is a pipe based USB device controller:
 *  - Pipe 0 is the default control pipe (DCP)
 *  - Pipes 1-9 are used for the remaining endpoints. Pipes 1 and 2 have
 *    256 byte buffers and support isochronous transfers, pipes 3-9 have
 *    64 byte buffers and support bulk/interrupt transfers.
 *  - The peripheral handles SETUP/STATUS staging of control transfers in
 *    hardware, the driver feeds the data stages through the CFIFO.
 *
 * This driver assigns pipes dynamically when endpoints are added by the
 * USBDevice stack (see USBPhyHw::endpoint_add).
 */
...
public:
    ...
    /* Called from the USBFS interrupt service routine */
    static void _usbisr(void);

private:
    /* Number of pipes including the default control pipe */
#define PIPE_NUM      (10)
#define USB_MIN_PIPE_NO (1u)
    ...
    /* Endpoint (including direction bit) assigned to each pipe */
    usb_ep_t pipe2ep[PIPE_NUM];
    uint16_t setup_buffer[4];
    ...
    volatile uint16_t *get_pipectr_reg(uint16_t pipe);
    volatile uint16_t *get_pipetre_reg(uint16_t pipe);
    volatile uint16_t *get_pipetrn_reg(uint16_t pipe);
    ...
    uint16_t EP2PIPE(usb_ep_t endpoint);
    usb_ep_t PIPE2EP(uint16_t pipe);
