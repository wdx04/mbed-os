...
{
    debug_if(ospi_api_c_debug, "ospi_prepare_command In: instruction.value %lx dummy_count %u address.bus_width %x address.disabled %x address.value %lx address.size %x\n",
...
extern void split_buffer_for_dma(const void *buffer, const size_t *length, size_t *pre_aligned_size, size_t *aligned_size, size_t *post_aligned_size);
